...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=dm-sans:400,500,600,700|dm-serif-display:400|space-grotesk:500,600,700" rel="stylesheet" />
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    @livewireStyles
</head>
<body
    x-data="{ loginOpen: @js(request()->routeIs('login') || old('login_modal') == 1) }"
    class="min-h-screen bg-paper font-sans text-ink selection:bg-accent selection:text-white"
>
        ...
        <nav class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-10" aria-label="Main navigation">
            ...
            <div class="flex items-center gap-3 text-sm font-semibold sm:gap-6">
                ...
                @else
                    <button
                        type="button"
                        x-on:click="loginOpen = true"
                        x-bind:aria-expanded="loginOpen"
                        aria-controls="login-dialog"
                        class="cursor-pointer rounded-full border border-ink px-4 py-2 hover:bg-ink hover:text-paper"
                    >Log in</button>
                @endauth
            </div>
        </nav>
